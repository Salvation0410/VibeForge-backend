...
import com.yupi.yuaicodemother.guardrail.PromptSafetyInputGuardrail;
import com.yupi.yuaicodemother.guardrail.RetryOutputGuardrail;
 ...
 * @author huang
 * @version 1.0
 * @description AI service创建工厂类 利用caffeine缓存防止重复创建AI Service和会话记忆
 * @date 2026/5/26
 */
@Configuration
@Slf4j
public class AiCodeGeneratorServiceFactory {
    ...
    private final Cache<String, AiCodeGeneratorService> serviceCache = Caffeine.newBuilder()
            ...
            .build();
    /**
     * 根据 appId 获取服务（带缓存）  默认采用HTML单文件生成 此处为了兼容历史逻辑
     */
    public AiCodeGeneratorService getAiCodeGeneratorService(long appId) {
        //先根据appId去缓存中获取Ai Service 没有则调用生成Ai Service的方法
    ...
    public AiCodeGeneratorService getAiCodeGeneratorService(long appId, CodeGenTypeEnum codeGenType) {
        ...
        //根据缓存键值获取缓存 没有则构建新的AI service服务
    ...
    private AiCodeGeneratorService createAiCodeGeneratorService(Long appId, CodeGenTypeEnum codeGenType) {
        ...
        switch (codeGenType) {
            case VUE_PROJECT -> {
                ...
                // 从数据库加载完整历史对话到缓存中，由于多了工具调用相关信息，加载的最大数量稍微多一些
                ...
                aiCodeGeneratorService = AiServices.builder(AiCodeGeneratorService.class)
                        ...
                        //处理工具运行结果时，如果调用的tool不存在，则返回错误信息 “幻觉”问题
