...
- Python 本轮 `compileall`、完整 `pytest` 和 `uv lock --check` 通过，其中 `pytest` 共 65 项测试通过并有 2 个依赖弃用警告。多数测试使用 Fake Model 和内存工具网关；新增 MockTransport 测试覆盖 Spring `BaseResponse` 非零业务码、协议畸形响应、错误脱敏、workflow `failed` 稳定错误码链路，以及旧版兼容响应仅接受精确 `data` envelope 的边界，仍不属于真实 Spring 或端到端验证。
